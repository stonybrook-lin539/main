...
::: solution
$G_1$ is a negative grammar that only generates *ab* and *ba*. $G_1$ \is $\setof{aa, bb}$.
$G_2$ is also a negative grammar that only generates *ab*, *aab* and so on. $G_2\is \setof{ {{{L}}}b, a{{{R}}}, bb, ba}$.
The corresponding combined gramamr is their union, which is $G \is \setof{aa, bb, {{{L}}}b, a{{{R}}}, ba}$.
The combined grammar will only generate *ab* which is also the intersection of the set $\setof{ab, ba}$ and $\setof{a^+b}$
:::
:::
...
::: solution
Since all symbols must be *a*, $\Sigma_E^n$ contains all lengths of *a* and the empty string.  
$\overline{G} = \Sigma_E^2 - +^G = {{{L}}}{{{R}}}
In other words, the negative grammar only contains the empty string which further means that only the empty string will be forbidden in this grammar. 
:::
:::
